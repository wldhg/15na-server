...
% Little modified by Jio Gim to use in IRONA Server.
%
function ret = read_bf_file_partial(datPath, redRes)
%% Open file
f = fopen(datPath, 'rb');
if (f < 0)
    error('Couldn''t open file %s', datPath);
    return;
end
...
while cur < (len - 3)
    ...
    fread(f,1); % default: uint8 is 1 byte size -- packet code -> don't need cuz I believe 15na-ap.
    cur = cur+3;

    % Believe! Because this may delivered through 15na-ap, so all the packet's code may 187.
